...
import { Text, View, TouchableOpacity, ActionSheetIOS, AlertIOS, Alert, Image, StatusBar } from 'react-native'
...
export default class ImageComment extends Component {
  ...
  showActionSheet() {
    ...
    (buttonIndex) => {
      switch (buttonIndex) {
        case 0:
          AlertIOS.prompt("Rename Image", null, this.handleRename, undefined, this.props.name)
          break;
        case 1:
          Alert.alert(
            'Confirm Deletion',
  ...
  handleRename(promptValue) {
    Snackbar.show({
      title: 'Image was succesfully renamed',
      duration: Snackbar.LENGTH_LONG,
    });
  }
  ...
  renderHeader(close) {
    ...
    let shareImage = {
      ...
      url: REACT_ICON,
      subject: this.props.name //  for email
    };
    return (
      <View style={styles.headerContainer}>
        <View style={styles.headerSection}>
          <TouchableOpacity onPress={close}>
            <Iconz style={[ styles.editButton, { marginRight: 20 } ]} name="close" size={22} />
          </TouchableOpacity>
        </View>
        <View style={styles.headerSection}>
          <TouchableOpacity onPress={() => Share.open(shareImage)}>
            <Iconz style={styles.editButton} name="share" size={18} />
          </TouchableOpacity>
          <TouchableOpacity onPress={this.showActionSheet}>
            <Iconz style={styles.editButton} name="settings" size={21} />
          </TouchableOpacity>
        </View>
      </View>
    );
  ...
  render() {
    return (
      <View style={styles.commentContainer}>
        ...
        <Lightbox style={{ alignSelf: "flex-start" }} backgroundColor="rgba(0, 0, 0, 0.8)" overlayColor="#fff" renderHeader={this.renderHeader} >
          <Image
            style={styles.image}
